...
SceneMenu::SceneMenu(void)
{
	...
	auto menuJson = resManager.GetJsonResource("MenuJson");
	selectIndex_ = 0;
	maxLayerNum_ = -1;
...
void SceneMenu::LoadJson(void)
{
	LoadLogo();
	LoadButton();
}

void SceneMenu::LoadLogo(void)
{
...
void SceneMenu::LoadButton(void)
{
...
SceneMenu::DrawStringInfo SceneMenu::LoadDrawStringInfo(nlohmann::json stringJson)
{
	DrawStringInfo ret;
...
std::vector<SceneMenu::ButtonInfo> SceneMenu::GetButtonInfoToLayer(int layerNum)
{
...
std::vector<SceneMenu::LogoInfo> SceneMenu::GetLogoInfoToLayer(int layerNum)
{
...
void SceneMenu::ChangeScene(void)
{
	std::string next = "";
	for (auto& button : buttonInfo_)
	{
	...
	{
		sceneManager.JumpScene(SceneManager::SCENE_ID::MAKE_SKIN);
		return;
	}
	else if (next == "Setting")
	{
		//現在実装なし
		return;
		sceneManager.JumpScene(SceneManager::SCENE_ID::SETTING);
		return;
	}
	else if (next == "Title")
	{
...
void SceneMenu::DrawButton(ButtonInfo button)
{
	//ボタンのテキスト描画
...
bool SceneMenu::ColCheckMouse(bool isCheakMove)
{
...
void SceneMenu::UpdateSelectIndex(void)
{
