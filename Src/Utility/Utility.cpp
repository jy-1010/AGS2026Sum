...
IntVector2 Utility::Lerp(const IntVector2& start, const IntVector2& end, float t)
{
    // 線形補間
    if (t >= 1.0f)
    {
...
IntVector2 Utility::Bezier(const IntVector2& p1, const IntVector2& p2, const IntVector2& p3, float t)
{
    IntVector2 a = Lerp(p1, p2, t);
    IntVector2 b = Lerp(p2, p3, t);
    return Lerp(a, b, t);
}
...
double Utility::Magnitude(const IntVector2& v)
{
...
int Utility::SqrMagnitude(const IntVector2& v)
{
...
double Utility::Distance(const IntVector2& v1, const IntVector2& v2)
{
...
VECTOR Utility::Normalize(const IntVector2& v)
{
    VECTOR ret = VGet(
...
bool Utility::IsPointInRect(const IntVector2& _pos, const IntVector2& _leftTop, const IntVector2& _rightBotm)
{
    //指定の範囲内に座標があるか調べる
...
bool Utility::IsPointInRectCircle(const IntVector2& _pos, const IntVector2& _circlePos, const float _radius)
{
...
FloatVector2 Utility::CalcSphericalUV(const VECTOR& normal)
{
    // --- 正規化（半径を除去） ---
